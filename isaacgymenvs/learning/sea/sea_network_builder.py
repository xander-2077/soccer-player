...
class A2CBuilder(network_builder.NetworkBuilder):
    ...
    class Network(network_builder.NetworkBuilder.BaseNetwork):
        ...
        def __init__(self, params, **kwargs):
            ...
            assert (
                len(input_shape["state_obs"]) == 1,
                "only flate state observation is supported",
            )
            assert (
                len(input_shape["state_history"]) == 1,
                "only flate state history is supported",
            )
            ...
            self.history_head = torch.nn.Linear(
                self.history_units[-1], out_history_shape
            )
            ...
            if False:
                ...
                self.actor_mlp_jit = torch.jit.load(actor_mlp_path, map_location=device)
                self.mu_act_jit = torch.jit.load(mu_act_path, map_location=device)
                self.mu_jit = torch.jit.load(mu_path, map_location=device)
        ...
        def forward(self, obs_dict):
            ...
            encode = self.history_encoder(state_history)
        ...
        def load(self, params):
            ...
            if self.has_history:
                ...
                self.history_norm_only_first = params["history"].get(
                    "norm_only_first_layer", False
                )
            else:
        ...
        def _calc_input_size_from_history(self, input_shape, history_layers=None):
            ...
            return (
                nn.Sequential(*history_layers)(torch.rand(1, *(input_shape)))
                .flatten(1)
                .data.size(1)
            )
